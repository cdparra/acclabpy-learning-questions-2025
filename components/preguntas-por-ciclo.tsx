...
interface PreguntasPorCicloProps {
  data: Pregunta[]
}

export function PreguntasPorCiclo({ data }: PreguntasPorCicloProps) {
  ...
  return (
    <div className="space-y-4">
      <Tabs value={activeCiclo} onValueChange={setActiveCiclo}>
        ...
        {ciclosUnicos.map((ciclo) => {
          const preguntasCiclo = getPreguntasPorCiclo(ciclo)
          const filteredPreguntas = searchTerm
            ? preguntasCiclo.filter(
                (p) =>
              ...
              <Card className="border-[#0057B8]/20 shadow-md">
                <CardHeader className="pb-2">
                  <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                    <div>
                      <CardTitle className="text-[#0057B8]">Preguntas del Ciclo {ciclo}</CardTitle>
                      <CardDescription>Total: {preguntasCiclo.length} preguntas</CardDescription>
                    </div>
                    <Input
                        ...
                        <AccordionItem key={pregunta.id} value={`item-${pregunta.id}`} className="border-[#0057B8]/20">
                          <AccordionTrigger className="hover:no-underline py-3">
                            <div className="flex flex-col sm:flex-row sm:items-center justify-between w-full pr-4 gap-2 text-left">
                              ...
                              <Badge
                                className={`ml-0 sm:ml-2 self-start sm:self-auto shrink-0 ${
                          ...
                          <AccordionContent>
                            <div className="space-y-4 p-4 bg-[#0057B8]/5 rounded-md">
                              ...
                              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                  <h4 className="font-semibold text-[#0057B8]">Ciclos que aportan:</h4>
                                  <p>{pregunta.ciclosAportan || "No especificado"}</p>
                                </div>
                              </div>
